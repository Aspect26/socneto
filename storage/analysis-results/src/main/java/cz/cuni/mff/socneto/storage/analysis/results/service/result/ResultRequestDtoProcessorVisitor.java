...
import cz.cuni.mff.socneto.storage.analysis.results.api.result.request.ResultRequestType;
...
public class ResultRequestDtoProcessorVisitor implements ResultRequestDtoVisitor<Result> {
    ...
    public Result requestResults(AggregationResultRequest resultRequest) {
        if (ResultRequestType.COUNT_PER_TIME == resultRequest.getResultRequestType()) {
            var timeCounts = aggregationsService.coutInTime(resultRequest.getJobId());
